...
export const schema = yup.object().shape({
  ...
  birthDate: yup.string().required(BIRTHDATE_ATTRIBUTES.errorMessages.REQUIRED),
  password: yup.string()
    .min(8, PASSWORD_ATTRIBUTES.errorMessages.INVALID)
    .required(PASSWORD_ATTRIBUTES.errorMessages.REQUIRED),
  matchedPassword: yup.string()
    .oneOf([yup.ref('password'), null], MATCHED_PASSWORD_ATTRIBUTES.errorMessages.INVALID)
